...
def forward(inputs,weights,function=sigmoid,step=-1):
	...
	if step == 0:
		...
		step = len(weights)-1
		f 	 = forward(inputs, weights, function, step)
		return function(np.dot(weights[step], np.append(1, f)))
	else:
		f 	 = forward(inputs, weights, function, step-1)
		return function(np.dot(weights[step-1], np.append(1, f)))
